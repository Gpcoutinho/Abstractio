...
O **Otto** é um polvo cientista curioso que estuda e organiza o oceano ao seu redor usando Python + POO.
...
|---|---|---|---|---|
| 1-0 | Antes de começar | Apresentação do Otto | — | Sem código. Narrativa já estabelecida na missão. |
| 1-1 | O que é POO? | Caderno de anotações de Otto | `Ada` | Otto anota observações em papéis soltos — Marta, Pedro e Joana fazem o mesmo, nas mesmas folhas. Caos de dados inconsistentes ("cor: rosa" / "pink" / código hex). O problema: dados e regras de uso vivem separados. A POO propõe juntar os dois — a ficha de Ada não é papel com dados, é um pacote completo. Ada já é referenciada pelo nome. Componentes: `{{caderno-abertura}}`, `{{caos-anotacoes}}`, `{{dados-globais}}`. |
| 1-2 | Objeto | Ada encontrada no oceano | `Ada`, `outra` | Otto *encontra* Ada no oceano. Não sabe ainda como ela foi criada — apenas observa que existe, tem identidade própria e é diferente de outros polvos. Metáfora da caixa preta. `outra` é usada para demonstrar identidade vs. igualdade (`==` vs `is`, `id()`). `ContaBancaria` aparece como exemplo do mercado de trabalho — mesmos três elementos (Identidade, Características, Ações). Nenhuma `class` aparece ainda. |
| 1-3 | Atributos | Ficha de observação de Ada | `Ada.nome`, `Ada.cor`, `Ada.num_tentaculos`, `Ada.tamanho_cm`, `Ada.especie` | Otto *lê* os atributos de Ada preenchendo a ficha de campo. Ele acessa dados de um objeto que já existe — não define nada ainda. Reforçar: atributo é informação que pertence ao objeto, cada polvo guarda os seus próprios valores. |
| 1-4 | Métodos | Otto assiste Ada agir | `Ada.nadar()`, `Ada.camuflar()`, `Ada.soltar_tinta()`, `Ada.capturar_presa(presa)` | Otto *observa* Ada executando comportamentos — ele chama os métodos e vê os resultados. Ainda não sabe como esses comportamentos foram definidos. Reforçar: método é o que um objeto sabe fazer, chamado de fora como uma instrução. |
| 1-5 | Classe | O grimório — de onde Ada veio? | `class Polvo`, `Ada`, `azulao`, `marinho` | **O grande reveal.** Após 3 missões usando Ada, a missão abre com a pergunta: *"mas de onde ele veio? Quem definiu que teria `nome`, `cor` e saberia `nadar()`?"* Otto abre o grimório e finalmente vê a `class Polvo`. Tom de descoberta — a classe não é ponto de partida, é a explicação de tudo que veio antes. |
| 1-6 | Construtor | Nascimento do polvo (instanciação) | `__init__`, `Polvo("Ada", "roxo", "Octopus vulgaris")` | Quando um polvo nasce (é instanciado), o `__init__` define seus valores iniciais — como uma certidão de nascimento oceânica emitida pelo próprio objeto. |
...
| 2-2 | Abstração | O que anotar no caderno de campo | `class Polvo` (simplificada), `class SistemaNervoso` (não abstraída) | Otto não registra cada célula do polvo no caderno — abstrai só o que é relevante para a pesquisa. A classe `Polvo` já é uma abstração: esconde a biologia real e expõe apenas o que importa. |
...
| 3-3 | Sobrecarga | Otto registra observações de formas diferentes | `registrar(nome)`, `registrar(nome, local="desconhecido")`, `registrar(nome, local, profundidade=0)` | Em Python, sobrecarga se faz com parâmetros padrão e `*args`. Otto pode registrar com só o nome do polvo, ou com localização, ou com profundidade — a mesma função aceita todas as formas. |
...
| 3-6 | Associação | Otto e a Caverna de Pesquisa | `class Otto`, `class CavernaSubmarina`, referência via atributo | Otto *usa* uma `CavernaSubmarina` — objetos independentes que se conhecem. Se um deixar de existir, o outro continua. Relação fraca entre dois objetos autônomos. |
...
| Missão | Conceito | Elemento marinho central | Nomes sugeridos | Núcleo do exemplo |
|---|---|---|---|---|
| 4-1 | Introdução ao nível | Otto como engenheiro da Base | — | Sem código principal. Otto agora projeta o software completo da Base de Pesquisa Submarina. |
...
- O protagonista é sempre **Otto** — polvo cientista, nunca "o programador" ou "o desenvolvedor".
- O polvo principal chama-se **Ada** (rosa, `Octopus vulgaris`, o que aparece na tela inicial).
