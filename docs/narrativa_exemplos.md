...
| 1-2 | Objeto | Polvinho (o indivíduo concreto) | `polvinho`, `azulao`, `class Polvo` | Polvinho é o objeto concreto: tem identidade, estado (`nome="Polvinho"`, `cor="roxo"`, `tamanho_cm=15`) e comportamentos (`nadar()`, `camuflar()`). Mexer em Polvinho não afeta outros polvos. |
| 1-3 | Classe | O caderno de campo de Polvonilson | `class Polvo`, `polvinho`, `azulao`, `marinho` | Polvonilson abre o caderno e escreve o **molde geral** de um Polvo. O caderno não é um polvo — é a instrução de como criá-los. A classe `Polvo` é esse molde. |
| 1-4 | Atributos | Ficha de observação de cada polvo | `nome`, `cor`, `num_tentaculos`, `tamanho_cm`, `especie` | A ficha de campo de cada polvo registra suas características: `nome`, `cor`, `num_tentaculos`, `tamanho_cm`, `especie`. São os dados que descrevem qualquer indivíduo da espécie. |
| 1-5 | Métodos | O que o polvo sabe fazer | `nadar()`, `camuflar()`, `soltar_tinta()`, `capturar_presa(presa)` | Comportamentos que qualquer polvo executa. Cada método é uma ação do objeto sobre o mundo (ou sobre si mesmo). |
