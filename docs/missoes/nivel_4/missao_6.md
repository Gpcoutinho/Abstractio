...
**Pergunta:** O padrão **Strategy** é mais adequado quando:

- [ ] Você precisa garantir que uma classe tenha apenas uma instância no sistema.
- [x] Você quer poder trocar o algoritmo ou comportamento de um objeto em tempo de execução sem alterar o código que o usa. ← correta
- [ ] Você precisa notificar múltiplos objetos quando o estado de outro muda.
- [ ] Você quer criar objetos complexos passo a passo.

**Explicação:** Strategy encapsula algoritmos intercambiáveis — o objeto que os usa não precisa saber qual estratégia está ativa, apenas chamá-la.
