...
**Pergunta:** Qual combinação representa o objetivo central do nível de arquitetura?

- [ ] Máximo de classes possível + mínimo de métodos por classe.
- [x] Alta coesão + baixo acoplamento + princípios SOLID. ← correta
- [ ] Usar todos os design patterns disponíveis em cada projeto.
- [ ] Evitar generics e abstrações para manter o código simples.

**Explicação:** Alta coesão (propósito único por classe) + baixo acoplamento (dependências via abstração) + SOLID é a fórmula do design orientado a objetos de qualidade.
