...
**Pergunta:** O que é Injeção de Dependência e como ela reduz o acoplamento?

- [ ] É uma técnica de herança que injeta métodos de uma classe em outra automaticamente.
- [x] É passar as dependências de uma classe de fora para dentro (via construtor ou método), em vez de criá-las internamente. ← correta
...
**Explicação:** Injeção de dependência desacopla a classe de suas implementações concretas — ela recebe o que precisa de fora, facilitando troca e testes.
