...
**Pergunta:** Em um sistema de biblioteca, livros existem antes e independente de uma coleção, e podem pertencer a várias coleções. Qual relacionamento modela isso?

- [ ] Composição, porque os livros fazem parte da coleção.
- [ ] Associação, porque livros e coleções não têm nenhuma relação.
- [x] Agregação, porque os livros existem independentemente e podem pertencer a mais de uma coleção. ← correta
- [ ] Herança, porque Livro herda comportamentos de Coleção.

**Explicação:** Livros existem independentemente e podem ser parte de múltiplas coleções — isso é agregação: relação "tem um" sem controle do ciclo de vida.
