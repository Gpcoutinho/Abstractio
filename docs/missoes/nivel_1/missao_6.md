...
**Pergunta:** O que acontece quando você executa `p = Produto('Caneta', 2.50)`?

- [ ] Nada — é preciso chamar `p.init()` manualmente depois.
- [x] O Python chama `__init__` automaticamente, inicializando os atributos do objeto. ← correta
...
**Explicação:** `__init__` é chamado automaticamente pelo Python no momento da criação do objeto. Você não precisa invocá-lo.
